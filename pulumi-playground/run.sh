...
PULUMI_STATE_DIR="$THIS_DIR/.pulumi-state"
PULUMI_APP_DIR="$THIS_DIR/src/pulumi_playground/pulumi_app"
...
function help {
    echo "$0 <task> <args>"
    echo "Tasks:"
    compgen -A function | grep -v ":" | cat -n
    echo "Pulumi Tasks:"
    compgen -A function | grep "pulumi:" | cat -n
}

# Set up Pulumi with local login and AWS profile
function pulumi:setup {
    # Create local state directory if it doesn't exist
    mkdir -p "$PULUMI_STATE_DIR"

    # Login to Pulumi using local file system
    pulumi login "file://$PULUMI_STATE_DIR"

    # Set AWS profile
    export AWS_PROFILE=pulumi-ui

    echo "Pulumi setup complete. Using local state at $PULUMI_STATE_DIR and AWS profile 'pulumi-ui'."
}

# Run Pulumi commands
function pulumi:run {
    pulumi:setup
    (cd "$PULUMI_APP_DIR" && pulumi $@)
}
